---
layout: post
title:  "Pruning deep neural networks to be fast and small"
...
There are a lot of papers about pruning, but I've never encountered pruning used in real life deep learning projects. Which is surprising considering all the industry effort on running deep learning on mobile devices.
...
First they state the pruning problem as an optimization problem: choose a subset of weights B, such that when pruning them the network cost change will be minimal.
...
Now to measure the effectiveness of other ranking methods, they compute the spearman correlation with the oracle. Surprise surprise, the ranking method they came up with (described bext) is most similar to the oracle.
...
But you really got to wonder - did they pose the pruning problem as minimizing the difference of the network costs, and **THEN** come up with the taylor expansion method,
